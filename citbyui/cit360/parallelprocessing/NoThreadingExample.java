 ...
 * @author buzzr
 */
public class NoThreadingExample implements Runnable {

    Random rng;

    public NoThreadingExample() {
        rng = new Random();
    }
    ...
    public static void main(String[] args) {
        NoThreadingExample inst = new NoThreadingExample();
        for (int i = 0; i < 6; i++) {
            inst.doStuff();
            //newThread();
        }
    ...
    public static void newThread() {
        Thread thread = new Thread(new NoThreadingExample());
        thread.start();
    }
